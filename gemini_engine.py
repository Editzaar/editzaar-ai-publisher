...
def live_web_search(query, max_results=6):
    """
    Scrapes live web search results from DuckDuckGo HTML to ground the AI with real facts.
    """
    try:
...
def call_gemini_api(prompt, system_instruction=""):
    """
    Calls Gemini 3.7/3.6 Flash model with automatic fallback.
    """
    payload = {
...
"""
    ...
    try:
        ...
        data = {
            ...
            "search_desc": desc_m.group(1) if desc_m else f"Master {topic}. Pro workflows by Editzaar."[:140],
    ...
    search_desc = data.get("search_desc", f"Master {topic}. Pro workflows by Editzaar.")[:145]
    html_body = data.get("html_body", "")

    # Generate 3D Graphic Thumbnail
...
def get_blogger_credentials():
    ...
    raise RuntimeError("No Blogger credentials found. Set BLOGGER_TOKEN_JSON environment variable or place token.json.")
